...
class Database:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('''
                CREATE TABLE IF NOT EXISTS scam_logs (
                    ...
                    evidence TEXT, -- JSON array of evidence URLs
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute('''
                INSERT INTO scam_logs (
                    id, reported_by, scammer_username, scammer_user_id,
                    scammer_additional_info, scam_type, scam_description,
    ...
    async def get_scam_log(self, log_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute('SELECT * FROM scam_logs WHERE id = ?', (log_id,)) as cursor:
    ...
    async def update_scam_log_status(self, log_id: str, status: str) -> bool:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                'UPDATE scam_logs SET status = ?, updated_at = ? WHERE id = ?',
                (status, now, log_id)
            )
            await db.commit()
